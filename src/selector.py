...
class OrangeChannelSelect(OrangeSelect):
    """Channel selector entity."""

    ENTITY_NAME = "input_source"
    SELECT_NAME = OrangeSelects.SELECT_CHANNEL
    ...
    def __init__(self, config_device: OrangeConfigDevice, device: client.OrangeTVClient):
        ...
        super().__init__(
            entity_id,
            {
                "en": f"{config_device.get_device_part()}Channel",
                "fr": f"{config_device.get_device_part()}Chaîne",
            },
            config_device,
            device,
            device.set_channel_by_name,
        )

    @property
    def current_option(self) -> str:
        """Return selector value."""
        return self._device.channel_name

    @property
    def select_options(self) -> list[str]:
        """Return selection list."""
        return self._device.channel_names
